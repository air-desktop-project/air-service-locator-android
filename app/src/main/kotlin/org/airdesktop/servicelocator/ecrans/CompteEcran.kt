...
/**
 * `POST /v1/appareils` : un appareil déjà enrôlé apporte la clé du nouveau, lue
 * d'un code affiché à l'écran. **Le geste n'est pas encore écrit** — ni la clé
 * matérielle, ni l'échange entre les deux téléphones.
 */
@Composable
fun EnrolerAppareilEcran(nav: NavController) {
    Scaffold(topBar = { Barre("Enrôler un appareil", nav) }) { marges ->
        Indisponible(
            Modifier.padding(marges), Icones.telephone, "Pas encore possible",
            "L'enrôlement d'un second appareil demande la clé matérielle et l'échange d'un code entre les deux téléphones. Ce sera écrit avec le transport.",
        )
    }
}
